    ...
    class FrmElegirRol
    {
       ...
       public FrmElegirRol()
        ...
        private void LblEntrar_Click(object sender, EventArgs e)
        {
            ...
            {
                var dialogAnswer = MessageBox.Show(string.Format("Se procederá a logear con el siguiente rol: {0}. Esta seguro?", rolSeleccionado.Descripcion), "Atención", MessageBoxButtons.YesNo);
                if (dialogAnswer == DialogResult.Yes)
                {
                    Hide();

                    //Seteo el rol en sesión
                    SessionManager.CurrentRol = rolSeleccionado;
